...
void Adafruit_Si4713::sendEmpty() {
    uint8_t dummy[1] = {0};
    // I2C_Master_Write(SI4710_ADDR0, dummy, 0); // zero-length write, nack
...
void Adafruit_Si4713::sendCommand(uint8_t len) {
  // Send command
  ser_print("writing command\n");
  I2C_Master_Write(SI4710_ADDR1, _i2ccommand, len);
  ser_print("write done\n");
  
  // Wait for status CTS bit
  // uint8_t status = 0;
  uint8_t buf[1] = {0};
  while (!(buf[0] & SI4710_STATUS_CTS))
    I2C_Master_Read(SI4710_ADDR1, buf, 1);
  ser_print("send command done\n");
}
...
uint8_t Adafruit_Si4713::getStatus() {
  ...
  I2C_Master_Read(SI4710_ADDR1, resp, 1);
  return resp[0];
}
