...
bool GGWave::computeFFTR(const float * src, float * dst, int Ni) {
    if (Ni != m_samplesPerFrame) {
        ggprintf("computeFFTR: N (%d) must be equal to 'samplesPerFrame' %d\n", Ni, m_samplesPerFrame);
        return false;
    }

    FFT(src, dst, Ni, m_rx.fftWorkI.data(), m_rx.fftWorkF.data());

    return true;
}

int GGWave::computeFFTR(const float * src, float * dst, int Ni, int * wi, float * wf) {
    if (wi == nullptr) return 2*Ni;
    if (wf == nullptr) return 3 + sqrt(Ni/2);

    FFT(src, dst, Ni, wi, wf);

    return 1;
}

int GGWave::filter(ggwave_Filter filter, float * waveform, int Ni, float p0, float p1, float * w) {
    if (w == nullptr) {
        switch (filter) {
            case GGWAVE_FILTER_HANN:                  return Ni;
            case GGWAVE_FILTER_HAMMING:               return Ni;
    ...
    if (w[0] == 0.0f && w[1] == 0.0f) {
        switch (filter) {
            case GGWAVE_FILTER_HANN:
                {
                    const float f = 2.0f*M_PI/(float)Ni;
                    for (int i = 0; i < Ni; i++) {
